    ...
    lines.forEach((line, idx) => {
      ...
      if (isPII && !isEncrypted && !isComment && isPropertyDecl) {
        ...
        findings.push({
          ...
          severity: 'CRÍTICA',
          law: 'Ley 21.719',
          article: 'Art. 18 (Medidas de seguridad)',
          file: filePath,
          lineNumber: lineNo,
      ...
      for (const { re, desc } of sqlInjectionPatterns) {
        if (re.test(line) && !line.trim().startsWith('//')) {
          findings.push({
            ...
            severity: 'CRÍTICA',
            law: 'Ley 21.719',
            article: 'Art. 18 (Seguridad) + Art. 20 (Notificación de brechas)',
            file: filePath,
            lineNumber: idx + 1,
            codeSnippet: line.trim(),
            recommendation:
    ...
    if (fileType === 'csharp' || fileType === 'typescript' || fileType === 'javascript') {
      ...
      if (createsUser && !hasConsent) {
        findings.push({
          ...
          severity: 'ALTA',
          law: 'Ley 21.719',
          article: 'Art. 4 (Consentimiento) + Art. 10 (Datos de menores)',
          file: filePath,
          recommendation:
      ...
      if (isController) {
        ...
        if (!hasDelete) {
          findings.push({
            ...
            severity: 'ALTA',
            law: 'Ley 21.719',
            article: 'Art. 7 (Derecho de Supresión / Cancelación)',
            file: filePath,
            recommendation:
        ...
        if (!hasExport) {
          findings.push({
            ...
            severity: 'ALTA',
            law: 'Ley 21.719',
            article: 'Art. 9 (Derecho de Portabilidad)',
            file: filePath,
            recommendation:
        ...
        if (!hasOptOut) {
          findings.push({
            ...
            severity: 'MEDIA',
            law: 'Ley 21.719',
            article: 'Art. 8 (Derecho de Oposición) + Art. 8ter (Bloqueo)',
            file: filePath,
            recommendation:
      ...
      for (const re of logWithPII) {
        if (re.test(line)) {
          findings.push({
            ...
            severity: 'ALTA',
            law: 'Ley 21.719',
            article: 'Art. 18 (Seguridad) + principio de minimización',
            file: filePath,
            lineNumber: idx + 1,
            codeSnippet: line.trim(),
            recommendation:
